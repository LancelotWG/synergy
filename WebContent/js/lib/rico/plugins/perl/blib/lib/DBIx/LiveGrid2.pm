...
sub new {
    ...
    $cgi ||= CGI->new;
    $vars      = \%cfg;
    for my $param( qw/id offset page_size get_total/ ) {
...
sub query_database {
    ...
    my $rows     = $self->get('ajax_page_size') + 1;
...
sub build_ajax_table {
    my($self,$table_aoa) = @_;
    my $table_data = '';
    my $rowcount   = @$table_aoa;
    $self->set("ajax_rowcount", $rowcount );
...
EOT
    my $rowcount=$self->get('ajax_rowcount');
    if ($rowcount < $self->get('ajax_page_size')) {
      my $totcnt=$rowcount+$self->get('ajax_offset');
